...
<xsl:template match="tomboy:note/tomboy:text/*[1]/text()[1]">h1. <xsl:value-of select="substring-before(., $newline)"/>
...
<xsl:template match="tomboy:italic">_<xsl:apply-templates select="node()"/>_</xsl:template>

<xsl:template match="tomboy:strikethrough">-<xsl:apply-templates select="node()"/>-</xsl:template>

<xsl:template match="tomboy:highlight">_<xsl:apply-templates select="node()"/>_</xsl:template>

<xsl:template match="tomboy:monospace">{{<xsl:apply-templates select="node()"/>}}</xsl:template>

<xsl:template match="size:small">
h5. <xsl:apply-templates select="node()"><xsl:with-param name="inhdr" select="1"/></xsl:apply-templates>
</xsl:template>

<xsl:template match="size:large">
h3. <xsl:apply-templates select="node()"><xsl:with-param name="inhdr" select="1"/></xsl:apply-templates>
</xsl:template>

<xsl:template match="size:huge">
h1. <xsl:apply-templates select="node()"><xsl:with-param name="inhdr" select="1"/></xsl:apply-templates>
</xsl:template>
